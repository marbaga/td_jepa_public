        ...
        "agent": {
            ...
            "model": {
                ...
                "archi": {
                    ...
                    "encoder": {"name": "BackwardArchi", "hidden_dim": 256, "hidden_layers": 2, "norm": True},
                    "rgb_encoder": {
                        "name": "drq",
                        "feature_dim": 256,
...
def sweep_pointmass():
    conf = {
        ...
        "agent": {
            "model": {"archi": {"z_dim": [50]}},
            "train": {"lr_encoder": [1e-4, 1e-5], "ortho_coef": [0.01, 0.1, 1], "discount": [0.99]},
        },
    }
    return conf
...
if __name__ == "__main__":
    ...
    main(args)
    # uv run -m scripts.pixel.launch_sptd_dmc --wandb_gname sptd_walker_v10 --sweep_config sweep_walker --use_wandb
    # uv run -m scripts.pixel.launch_sptd_dmc --wandb_gname sptd_cheetah_v10 --sweep_config sweep_cheetah --use_wandb
    # uv run -m scripts.pixel.launch_sptd_dmc --wandb_gname sptd_quadruped_v10 --sweep_config sweep_quadruped --use_wandb
    # uv run -m scripts.pixel.launch_sptd_dmc --wandb_gname sptd_pointmass_v10 --sweep_config sweep_pointmass --use_wandb
