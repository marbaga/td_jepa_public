    ...
    "agent": {
        ...
        "model": {
            ...
            "archi": {
                ...
                "phi_mlp_encoder": {"name": "BackwardArchi", "hidden_dim": 256, "hidden_layers": 2, "norm": True},
                "rgb_encoder": {
                    "name": "drq",
                    "feature_dim": 256,
                },
...
def sweep_pointmass():
    conf = {
        ...
        "agent": {
            "train": {"lr_phi": [1e-4, 1e-5], "phi_ortho_coef": [0.01, 0.1, 1], "discount": [0.99]},
        },
    }
    return conf
...
if __name__ == "__main__":
    ...
    main(args)
    # uv run -m scripts.train.pixel.launch_td_jepa_sym_dmc --use_wandb --wandb_gname td_jepa_sym_walker_pixel --data_path datasets --workdir_root results --sweep_config sweep_walker
    # uv run -m scripts.train.pixel.launch_td_jepa_sym_dmc --use_wandb --wandb_gname td_jepa_sym_cheetah_pixel --data_path datasets --workdir_root results --sweep_config sweep_cheetah
    # uv run -m scripts.train.pixel.launch_td_jepa_sym_dmc --use_wandb --wandb_gname td_jepa_sym_quadruped_pixel --data_path datasets --workdir_root results --sweep_config sweep_quadruped
    # uv run -m scripts.train.pixel.launch_td_jepa_sym_dmc --use_wandb --wandb_gname td_jepa_sym_pointmass_pixel --data_path datasets --workdir_root results --sweep_config sweep_pointmass
